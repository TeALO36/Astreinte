...
test("deux interlocuteurs simultanés ne se mélangent jamais", async () => {
    ...
    assert.ok(a.turns.every((t) => t.content.includes("alice")), `contamination dans le contexte d'Alice : ${JSON.stringify(a.turns.map((t) => t.content))}`);
    assert.ok(b.turns.every((t) => t.content.includes("bob")), `contamination dans le contexte de Bob : ${JSON.stringify(b.turns.map((t) => t.content))}`);
...
test("les tours anciens sont repliés dans un résumé plutôt que jetés", async () => {
    ...
    await store.withContact("eve", "Eve", (ctx) => {
        for (let i = 0; i < 10; i++)
            store.addTurn(ctx, "user", `message ${i}`);
    });
...
function ctxWith(turnCount, escalated = false) {
    return {
        contactId: "x",
        turns: [],
        turnCount,
...
//# sourceMappingURL=snap-astreinte.test.js.map