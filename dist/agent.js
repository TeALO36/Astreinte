...
export class Agent {
    ...
    buildSystemPrompt(ctx) {
        ...
        if (name)
            parts.push(`Tu t'appelles ${name}.`);
        const lang = this.cfg.str("persona.language");
        if (lang === "fr")
            parts.push("Réponds toujours en français.");
        else if (lang === "en")
            parts.push("Always answer in English.");
        else
            parts.push("Réponds dans la langue du message que tu reçois.");
        if (this.cfg.bool("persona.disclose_ai")) {
            parts.push("Si on te demande si tu es un humain, un robot ou une IA, réponds honnêtement que tu es un assistant automatique. Ne prétends jamais être une personne.");
        }
        const limits = this.policy.promptFragment();
        if (limits)
            parts.push(limits);
        if (ctx.contactName)
            parts.push(`Ton interlocuteur s'appelle ${ctx.contactName}.`);
        if (ctx.summary) {
            parts.push(`Rappel des échanges plus anciens avec cette personne :\n${ctx.summary}`);
        }
        parts.push("Tu écris dans une messagerie : pas de titres, pas de listes à puces longues, pas de mise en forme markdown. Des phrases courtes, comme un message qu'on lit sur un téléphone.");
        return parts.join("\n\n");
    }
    messagesFor(ctx, incoming) {
        const msgs = [{ role: "system", content: this.buildSystemPrompt(ctx) }];
    ...
    wantsVoice(incoming, reply) {
        const mode = this.cfg.str("voice.mode");
        if (mode === "never")
            return false;
        if (!this.tts.available || !this.transport.capabilities.voice)
            return false;
        if (!this.tts.fits(reply))
            return false;
        if (mode === "always")
            return true;
            ...
            try {
                ...
                if (this.wantsVoice(incoming, reply)) {
                    try {
                        ...
                        console.error(`[snap-astreinte] vocal indisponible, repli texte : ${e.message}`);
                        entry.reason = `vocal indisponible : ${e.message}`;
    ...
    async flushPending() {
        ...
        for (const stored of this.store.list()) {
            if (!stored.pendingText || stored.escalated)
                continue;
