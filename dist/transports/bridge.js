...
export class BridgeTransport {
    opts;
    id;
    capabilities = { voice: false, typing: false };
    running = false;
    loop = null;
    controller = null;
    constructor(opts) {
        ...
        if (!opts.baseUrl.trim()) {
            throw new TransportError("adresse du pont absente. Renseignez l'URL locale exposée par votre pont dans la configuration de l'extension.");
        }
    }
    url(path) {
    ...
    async probe() {
        try {
            ...
            if (!res.ok)
                return { ok: false, voice: false, detail: `HTTP ${res.status}` };
            const body = (await res.json());
            return { ok: body.ok !== false, voice: body.voice === true, detail: body.detail };
        }
        catch (e) {
            return { ok: false, voice: false, detail: e.message };
        }
    }
    async start(handler) {
        const health = await this.probe();
        if (!health.ok) {
            throw new TransportError(`le pont ${this.opts.baseUrl} ne répond pas (${health.detail ?? "injoignable"}). ` +
        ...
        const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error("le flux /events ne s'est pas ouvert")), 10_000).unref?.());
        try {
            ...
            throw new TransportError(`${this.opts.baseUrl} : ${e.message}`);
        }
        console.error(`[snap-astreinte] pont ${this.id} connecté (${this.opts.baseUrl})` +
            `${health.voice ? ", vocal disponible" : ", vocal indisponible"}` +
            `${health.detail ? ` — ${health.detail}` : ""}`);
    }
    async pump(handler, onReady) {
        ...
        while (this.running) {
            ...
            try {
                ...
                if (this.running) {
                    console.error(`[snap-astreinte] flux du pont interrompu : ${e.message}`);
