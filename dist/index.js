...
async function daemon() {
    ...
    try {
        ...
        console.error(`[snap-astreinte] ${e.message}`);
        process.exit(1);
    }
    const agent = new Agent(cfg, transport);
    const purged = agent.store.purge(cfg.num("context.retention_days"));
    if (purged)
        console.error(`[snap-astreinte] ${purged} conversation(s) purgée(s) par ancienneté`);
    try {
    ...
    console.error(`[snap-astreinte] permanence active — garde-fous ${cfg.bool("limits.enabled") ? "actifs" : "DÉSACTIVÉS"}` +
        `, voix ${cfg.str("voice.mode")}`);
    const flush = setInterval(() => {
        if (!withinActiveHours(cfg.str("limits.active_hours")))
            return;
        agent
            .flushPending()
            .then((n) => {
            if (n)
                console.error(`[snap-astreinte] ${n} message(s) en attente traité(s)`);
        })
...
async function check() {
    const cfg = Config.load();
    const lines = [`Données      : ${astreinteHome()}`];
    ...
    try {
        ...
        lines.push(`Modèle       : PROBLÈME — ${e.message}`);
    }
    ...
    if (!tts.available) {
        ...
        try {
            ...
            lines.push(`Voix         : PROBLÈME — ${e.message}`);
        }
    }
