...
export class Tts {
    ...
    get available() {
        if (this.mode === "disabled")
            return false;
        if (this.mode === "http")
            return !!this.cfg.str("voice.tts_url").trim();
    ...
    async synthesize(text) {
        if (!this.available)
            throw new TtsError("aucun moteur de synthèse configuré");
    ...
    async viaHttp(text) {
        ...
        try {
            ...
            if (audio.length < 64)
                throw new TtsError("le serveur TTS a renvoyé un fichier vide");
            ...
            if (e.name === "AbortError")
                throw new TtsError("le serveur TTS n'a pas répondu à temps");
            throw new TtsError(`${e.message} (${url})`);
        }
        finally {
            clearTimeout(timer);
        }
    }
    async viaCommand(text) {
        ...
        try {
            ...
            const parts = splitCommand(template.replaceAll("{out}", out).replaceAll("{ref}", ref).replaceAll("{text}", text));
            const bin = parts[0];
            if (!bin)
                throw new TtsError("commande TTS vide");
            await new Promise((resolveDone, rejectDone) => {
                ...
                child.on("close", (code) => {
                    if (code === 0)
                        ...
                        rejectDone(new TtsError(`${bin} a terminé avec le code ${code} : ${stderr.slice(-300)}`));
                });
                if (usesStdin) {
            ...
            if (!existsSync(out))
                throw new TtsError(`la commande n'a produit aucun fichier (${out})`);
            const audio = readFileSync(out);
            if (audio.length < 64)
                throw new TtsError("le fichier audio produit est vide");
 ...
 */
export function asksForVoice(text) {
    const t = text
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
