...
function emptyContext(contactId, contactName) {
    const now = Date.now();
    return {
        contactId,
        contactName,
...
export class ContactStore {
     ...
     */
    async withContact(contactId, contactName, fn) {
        ...
        const run = previous.catch(() => undefined).then(async () => {
            ...
            if (contactName && ctx.contactName !== contactName)
    ...
    trim(ctx) {
        ...
        if (ctx.summary.length > 4000)
            ctx.summary = ctx.summary.slice(-4000);
    }
    addTurn(ctx, role, content) {
