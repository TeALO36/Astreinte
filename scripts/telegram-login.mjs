...
const sessionFile = process.env.TELEGRAM_SESSION_FILE ?? ".telegram/session.txt";

if (!Number.isInteger(apiId) || apiId <= 0 || !apiHash) {
  console.error(
    "Set TELEGRAM_API_ID and TELEGRAM_API_HASH first. Create them at https://my.telegram.org.",
  );
  process.exit(1);
}
...
try {
  ...
  console.log(`Telegram session saved to ${sessionFile}. Keep it private.`);
} finally {
  rl.close();
