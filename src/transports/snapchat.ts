...
export function createSnapchatTransport(opts: Partial<BridgeOptions>): BridgeTransport {
  const baseUrl = (opts.baseUrl ?? "").trim() || SNAPCHAT_BRIDGE_DEFAULT_URL;
  if (!opts.baseUrl?.trim()) {
    console.error(
      `[snap-astreinte] aucune adresse de pont Snapchat configurée, essai sur ${SNAPCHAT_BRIDGE_DEFAULT_URL}`,
    );
  }
...
export function snapchatSetupHint(baseUrl: string): string {
  return [
