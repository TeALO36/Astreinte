...
export function withinActiveHours(spec: string, now = new Date()): boolean {
  const trimmed = spec.trim();
  if (!trimmed) return true;
  const m = trimmed.match(/^(\d{1,2}):(\d{2})\s*-\s*(\d{1,2}):(\d{2})$/);
  if (!m) return true; // Une plage mal écrite ne doit pas rendre l'assistant muet.
  const [, h1, m1, h2, m2] = m;
...
export class Policy {
  ...
  evaluate(ctx: ContactContext, incoming: string): PolicyVerdict {
    if (!this.enabled) return { action: "reply" };
  ...
  clamp(reply: string): string {
    if (!this.enabled) return reply;
    const max = this.cfg.num("limits.max_reply_chars");
    if (max <= 0 || reply.length <= max) return reply;
  ...
  promptFragment(): string {
    ...
    const parts: string[] = [];
    if (scope) parts.push(`Tu traites uniquement : ${scope}`);
    if (forbidden) {
      parts.push(
        `Tu ne traites pas et tu passes la main pour : ${forbidden}\n` +
          `Dans ces cas, dis simplement que tu transmets, sans improviser de solution.`,
      );
    }
    const max = this.cfg.num("limits.max_reply_chars");
    if (max > 0) parts.push(`Réponses courtes, ${max} caractères maximum.`);
    return parts.join("\n\n");
  }
}
